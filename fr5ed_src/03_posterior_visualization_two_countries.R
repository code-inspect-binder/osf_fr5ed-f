...
pdf.r<-1.05
tiff("posterior_two_countries.tif",width=(w1/rat[1])*sum(rat)*pdf.r*1.2,height=39*pdf.r,units="cm",res=600)
...
plot(NULL,ylim=c(const2,0),xlim=c(-1,1),type="n",xaxs="i",yaxs="i",xaxt="n",yaxt="n",bty="n")
text(0,const2-0.8,"Posterior distributions India",font=2)
...
restab<-data.frame(parameter=rownames(restab),regression=paste(describe[,4],describe[,5],describe[,6],describe[,7]),restab)
